...
function restoreCookies(obj) { if (obj && typeof obj === 'object') for (const [k,v] of Object.entries(obj)) cookieJar[k]=v; }
...
async function main() {
  ...
  let totalPages = null;
  let totalCount = null;
  const seen = new Set(); // comment IDs
  ...
  if (hasResume) {
    try {
      ...
      for (const c of comments) if (c && typeof c === 'object' && c.id != null) seen.add(c.id);
      console.log(`Resuming from page ${page} (already have ${comments.length} comments)`);
    } catch (e) {
  ...
  while (true) {
    ...
    let data;
    try { data = await fetchPage(page); } catch (err) {
    ...
    if (!totalPages) {
      ...
      console.log(`Discovered total pages=${totalPages}, total comments=${totalCount}`);
    }

    const batch = data.comments || data.items || [];
    if (batch.length === 0) {
      console.log('No comments in response, stopping.');
      break;
    }

    let added = 0;
    for (const c of batch) {
      const id = c && typeof c === 'object' ? (c.id ?? c.uuid ?? c._id) : null;
      if (id != null) {
        if (!seen.has(id)) {
          seen.add(id);
          comments.push(c);
      ...
      } else { // fallback: push if not already (rare)
        comments.push(c);
        added++;
      }
    }

    console.log(`Page ${page}: received ${batch.length}, added ${added}, total stored ${comments.length}`);
